...
- Both packages are versioned together via `fixed` in `.changeset/config.json` — a single changeset bumps both.
- To release: run `pnpm changeset`, select packages and bump type, commit the generated file, push to master. GitHub Actions creates a "Version Packages" PR automatically. Merging that PR triggers the publish.
- Do not run `pnpm publish` or create git tags manually — changesets handles both.
